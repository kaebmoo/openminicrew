...
- telemetry ของ receipt OCR (`ocr_telemetry`) เป็นแบบนิรนาม: เก็บ SHA-256 hash ของ user id ที่ใส่ salt (`OCR_TELEMETRY_SALT`) พร้อมตัวเลข confidence/action เท่านั้น — ไม่เก็บ user id ดิบ และไม่เก็บเนื้อหาใบเสร็จ (ดู CONFIGURATION.md หัวข้อ "OCR telemetry salt")
