...
async def handle_start(user_id: str, user: dict, args: str, **kw) -> _RESULT:
    ...
    welcome = (
        ...
        "• /consent chat on — อนุญาตให้เก็บ chat history ตาม retention\n"
        "• /consent location on — อนุญาตให้บันทึก location แบบ plaintext อายุสั้นตาม TTL\n"
        "• /authgmail — ให้สิทธิ์ Gmail ผ่าน OAuth เมื่อพร้อมใช้งาน\n\n"
        "พิมพ์ /privacy หรือ /consent เพื่อดูสถานะปัจจุบัน แล้วค่อยเริ่มใช้งานต่อ"
    )
    return _ok(welcome)
